...
function handleExplodeAction(row,col,game,cellEl,tableEl){
    ...
    panelEl.innerHTML=` <h3>LOSE(╯︵╰)总用时${game.seconds}秒哦</h3>`;
...
function handleWin(row,col,game,cellEl,tableEl){
    ...
    gameEl.append(panelEl);
    panelEl.innerHTML=` <h3>WIN(*￣︶￣)总用时${game.seconds}秒哦</h3>`;
...
function renderWelcome(){
    ...
    <button id='intermediate'>中级</button>
    <button id='advance'>高級</button>
    </div>
    `;
    let buttonEl=gameEl.querySelector('button#level0');
    buttonEl.onclick=()=>{    
        let game = new Game(8, 8, 0.15);
    game.shuffle();
    renderTable(game);
    }
    buttonEl=gameEl.querySelector('button#intermediate');
    buttonEl.onclick=()=>{
    let game = new Game(14, 26, 0.15);
    game.shuffle();
    renderTable(game);
    }
